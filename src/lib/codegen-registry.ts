...
`,
  ...
  "matic-email/send-email": `import { fetchCredentials } from "./lib/credential-helper";
...
export async function sendEmailStep(
  ...
  "use step";
  const credentials = await fetchCredentials("matic-email");
  ...
  try {
    if (input.recipientType === "applicant") {
      ...
      if (reviewersResponse.ok) {
        ...
        recipients = reviewersData.map((r: any) => r.email).filter(Boolean);
      }
    ...
    if (input.includeApplicationLink === "true") {
      const portalResponse = await fetch(\`\${apiUrl}/api/v1/forms/\${formId}\`, {
        headers: {
          "Content-Type": "application/json",
          ...(credentials.MATIC_API_KEY && {
            Authorization: \`Bearer \${credentials.MATIC_API_KEY}\`,
...
`,
  ...
  "matic-email/send-notification": `import { fetchCredentials } from "./lib/credential-helper";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        error:
          errorData.error || \`Failed to send notification: \${response.status}\`,
      };
    }
...
`,
  ...
  "matic-email/send-to-reviewers": `import { fetchCredentials } from "./lib/credential-helper";
...
`,
  ...
  "matic-review/move-to-stage": `import { fetchCredentials } from "./lib/credential-helper";
...
type MoveToStageResult =
  | {
      success: true;
      data: { applicationId: string; stageId: string; stageName: string };
    }
  | { success: false; error: { message: string } };
...
export async function moveToStageStep(
  ...
  "use step";
  const credentials = await fetchCredentials("matic-review");
  const apiUrl = credentials.MATIC_API_URL;
  const formId = credentials.MATIC_FORM_ID;
...
`,
  ...
  "matic-review/assign-reviewers": `import { fetchCredentials } from "./lib/credential-helper";
...
`,
  ...
  "matic-review/add-tags": `import { fetchCredentials } from "./lib/credential-helper";
...
export async function addTagsStep(
...
`,
  ...
  "matic-review/remove-tags": `import { fetchCredentials } from "./lib/credential-helper";
...
`,
  ...
  "matic-review/set-status": `import { fetchCredentials } from "./lib/credential-helper";
...
`,
  ...
  "matic-review/get-application": `import { fetchCredentials } from "./lib/credential-helper";
...
`,
  ...
  "matic-review/check-score": `import { fetchCredentials } from "./lib/credential-helper";
