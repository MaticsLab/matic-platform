...
import type { generateImageStep } from "@/lib/workflow/plugins/ai-gateway/steps/generate-image";
import type { generateTextStep } from "@/lib/workflow/plugins/ai-gateway/steps/generate-text";
import type { firecrawlScrapeStep } from "@/lib/workflow/plugins/firecrawl/steps/scrape";
import type { firecrawlSearchStep } from "@/lib/workflow/plugins/firecrawl/steps/search";
import type { createTicketStep } from "@/lib/workflow/plugins/linear/steps/create-ticket";
import type { sendEmailStep } from "@/lib/workflow/plugins/resend/steps/send-email";
import type { sendSlackMessageStep } from "@/lib/workflow/plugins/slack/steps/send-slack-message";
...
export const stepRegistry: Record<string, StepFunction> = {
  ...
  "Send Slack Message": async (input) =>
    (
      await import("@/lib/workflow/plugins/slack/steps/send-slack-message")
    ).sendSlackMessageStep(input as Parameters<typeof sendSlackMessageStep>[0]),
  "Create Ticket": async (input) =>
    (await import("@/lib/workflow/plugins/linear/steps/create-ticket")).createTicketStep(
  ...
  "Generate Text": async (input) =>
    (
      await import("@/lib/workflow/plugins/ai-gateway/steps/generate-text")
    ).generateTextStep(input as Parameters<typeof generateTextStep>[0]),
  "Generate Image": async (input) =>
    (
      await import("@/lib/workflow/plugins/ai-gateway/steps/generate-image")
    ).generateImageStep(input as Parameters<typeof generateImageStep>[0]),
  Scrape: async (input) =>
    (await import("@/lib/workflow/plugins/firecrawl/steps/scrape")).firecrawlScrapeStep(
      input as Parameters<typeof firecrawlScrapeStep>[0]
    ),
  Search: async (input) =>
    (await import("@/lib/workflow/plugins/firecrawl/steps/search")).firecrawlSearchStep(
