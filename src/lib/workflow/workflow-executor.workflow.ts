...
import type { WorkflowEdge, WorkflowNode } from "@/lib/workflow/workflow-store";
...
async function executeActionStep(input: {
  ...
  if (actionType === "Condition") {
    ...
    const stepModule = await systemAction.importer();
    ...
    return await stepModule[systemAction.stepFunction]({
  ...
  if (systemAction) {
    const stepModule = await systemAction.importer();
    const stepFunction = stepModule[systemAction.stepFunction];
  ...
  if (stepImporter) {
    const stepModule = await stepImporter.importer();
    const stepFunction = stepModule[stepImporter.stepFunction];
    if (stepFunction) {
      return await stepFunction(stepInput);
    }
