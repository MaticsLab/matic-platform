...
export const currentWorkspaceIdAtom = atom<string | null>(null); // Current workspace context
...
export const autosaveAtom = atom(
  ...
  async (get, set, options?: { immediate?: boolean }) => {
    ...
    const saveFunc = async () => {
      try {
        await automationWorkflowsClient.update(workflowId, { 
          nodes: nodes as any, 
          edges: edges as any 
        });
...
export const loadWorkflowAtom = atom(
  ...
  async (get, set, workflowId: string) => {
    try {
      ...
      set(nodesAtom, workflow.nodes as WorkflowNode[]);
      set(edgesAtom, workflow.edges as WorkflowEdge[]);
      set(currentWorkflowIdAtom, workflow.id);
      set(currentWorkflowNameAtom, workflow.name);
      set(currentWorkflowVisibilityAtom, workflow.visibility as WorkflowVisibility);
      set(isWorkflowOwnerAtom, workflow.is_owner);
    } catch (error) {
