...
import { authClient, signIn, signUp } from '@/auth/client/main'
...
function AuthPageContent() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    // If login mode with no password, use magic link
    if (isLogin && !password) {
                ...
                <form onSubmit={handleEmailAuth} className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="password" className="block text-sm font-medium text-gray-700 mb-2">
                      Password <span className="text-gray-500 font-normal">(optional - leave empty for magic link)</span>
                    </label>
                    <Input
                      id="password"
                      type="password"
                      ...
                      placeholder={isLogin ? "Leave empty to use magic link" : "Create a password or leave empty for magic link"}
                      className="w-full h-12 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                      minLength={6}
                    />
                  </div>
                  ...
                  >
                    {isLoading ? (
                      ...
                      !password && isLogin ? 'Send Magic Link' : (isLogin ? 'Log in with email' : 'Sign up with email')
                    )}
                  </Button>

                  {magicLinkSent && (
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">
                      ✓ Check your email! We sent you a login link to <strong>{email}</strong>
                    </div>
                  )}
                </form>
