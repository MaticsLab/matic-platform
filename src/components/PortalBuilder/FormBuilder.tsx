...
import { 
  ...
  Upload, MessageSquare, Lightbulb, FileText, AlertCircle, CheckCircle, AlertTriangle, HelpCircle, EyeOff, UserPlus, Send
} from 'lucide-react'
...
function FieldEditor({ 
  ...
  const renderFieldPreview = () => {
    ...
    switch (field.type) {
      ...
      case 'recommendation':
        const numRecommenders = field.config?.numRecommenders || 2
        return (
          <div className="border border-gray-200 rounded-xl bg-gradient-to-br from-purple-50 to-white p-4 space-y-3">
            <div className="flex items-center gap-2 text-purple-700">
              <UserPlus className="w-5 h-5" />
              <span className="text-sm font-medium">Letter of Recommendation Request</span>
            </div>
            <p className="text-xs text-gray-500">
              Applicants will add {numRecommenders} recommender{numRecommenders !== 1 ? 's' : ''} who will receive email requests
            </p>
            <div className="space-y-2">
              {Array.from({ length: Math.min(numRecommenders, 2) }).map((_, i) => (
                <div key={i} className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded-lg">
                  <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center">
                    <UserPlus className="w-4 h-4 text-purple-600" />
                  </div>
                  <div className="flex-1">
                    <div className="text-xs text-gray-400">Recommender {i + 1}</div>
                    <div className="text-sm text-gray-600">name@email.com</div>
                  </div>
                  <Send className="w-4 h-4 text-gray-300" />
                </div>
              ))}
              {numRecommenders > 2 && (
                <p className="text-xs text-gray-400 text-center">+{numRecommenders - 2} more recommender{numRecommenders > 3 ? 's' : ''}</p>
              )}
            </div>
          </div>
        )
