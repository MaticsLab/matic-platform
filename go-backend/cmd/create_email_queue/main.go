...
func main() {
	...
	if err := godotenv.Load(); err != nil {
		// Try loading from parent if not found
	...
	log.Println("Creating email_queue table if it doesn't exist...")

	// Create the email_queue table using the SQL from migration 032
	sql := `
...
`
	...
	if err := database.DB.Exec(sql).Error; err != nil {
		log.Fatalf("Failed to create email_queue table: %v", err)
	}

	log.Println("✅ email_queue table created successfully!")
}
