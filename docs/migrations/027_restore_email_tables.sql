-- Migration 027: Restore Email/Gmail Integration Tables
-- Description: Recreates tables for Gmail OAuth connections that were dropped in migration 019
-- The email functionality is still needed for the Communications Center

BEGIN;

-- Gmail OAuth Connections (per workspace, can have multiple accounts)
CREATE TABLE IF NOT EXISTS gmail_connections (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    workspace_id UUID NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
    user_id VARCHAR(255) NOT NULL,  -- Supabase user ID who connected
    ...
    send_permission VARCHAR(50) DEFAULT 'myself' CHECK (send_permission IN ('myself', 'admins', 'members', 'everyone', 'specific')),
    allowed_user_ids JSONB DEFAULT '[]',
    is_default BOOLEAN DEFAULT false,
    needs_reconnect BOOLEAN DEFAULT false,
    reconnect_reason TEXT DEFAULT '',
