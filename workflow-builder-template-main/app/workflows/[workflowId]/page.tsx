...
const WorkflowEditor = ({ params }: WorkflowPageProps) => {
  ...
  const loadExistingWorkflow = useCallback(async () => {
    // Check if this is an embedded workflow (stored in sessionStorage)
    if (isEmbedded || workflowId.startsWith('embedded-')) {
      const storedWorkflow = sessionStorage.getItem(`matic-workflow-${workflowId}`);
      if (storedWorkflow) {
        try {
          ...
          console.error("Failed to parse embedded workflow:", error);
        }
      }
      // If no stored workflow found for embedded, start fresh
      setCurrentWorkflowId(workflowId);
