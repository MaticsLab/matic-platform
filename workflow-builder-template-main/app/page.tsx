...
const Home = () => {
  ...
  const { isEmbedded, workspaceId, formId } = useAuthContext();
  ...
  const [embeddedWorkflowId] = useState(() => `embedded-${nanoid()}`);
  ...
  useEffect(() => {
    const createWorkflowAndRedirect = async () => {
      ...
      try {
        // In embedded mode, use local workflow ID without database persistence
        if (isEmbedded) {
          // Store workflow data in sessionStorage for embedded mode
          const embeddedWorkflow = {
            id: embeddedWorkflowId,
          ...
          sessionStorage.setItem(
            `matic-workflow-${embeddedWorkflowId}`,
            JSON.stringify(embeddedWorkflow)
          );

          // Set flags for sidebar animation
          sessionStorage.setItem("animate-sidebar", "true");
          setIsTransitioningFromHomepage(true);

          // Navigate to embedded workflow editor
          console.log("[Homepage] Navigating to embedded workflow page");
          router.replace(`/workflows/${embeddedWorkflowId}?embedded=true&workspace_id=${workspaceId || ''}&form_id=${formId || ''}`);
          return;
        }

        // Standard flow: create via API
        await ensureSession();
  ...
  }, [nodes, edges, router, ensureSession, setIsTransitioningFromHomepage, isEmbedded, embeddedWorkflowId, workspaceId, formId]);
